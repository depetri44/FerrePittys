...
const CartItem = ({ product, name, seller, price, cuttedPrice, image, stock, quantity, inCart }) => {
    ...
    const increaseQuantity = (id, quantity, stock) => {
        ...
        if (quantity >= stock) {
            enqueueSnackbar("Cantidad máxima de pedido", { variant: "warning" });
            return;
        };
    ...
    const saveForLaterHandler = (id) => {
        ...
        removeCartItem(id);
        enqueueSnackbar("GUARDAR PARA MÁS ADELANTE", { variant: "success" });
    }
    ...
    return (
        <div className="flex flex-col gap-3 py-5 pl-2 sm:pl-6 border-b overflow-hidden" key={product}>
                ...
                <div className="flex flex-col sm:gap-5 w-full pr-6">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start pr-5 gap-1 sm:gap-0">
                        <div className="flex flex-col gap-0.5 sm:w-3/5">
                            ...
                            <span className="text-sm text-gray-500">Vendedor/a: {seller}</span>
                        </div>

                        <div className="flex flex-col sm:gap-2">
                            <p className="text-sm">Entrega en {getDeliveryDate()} | <span className="text-primary-green">Gratis</span> <span className="line-through">${quantity * 40}</span></p>
                            <span className="text-xs text-gray-500">7 Política de reemplazo de días</span>
                        </div>
            ...
            <div className="flex justify-between pr-4 sm:pr-0 sm:justify-start sm:gap-6">
                ...
                {inCart && (
                    <>
                    <button onClick={() => saveForLaterHandler(product)} className="sm:ml-4 font-medium hover:text-primary-blue">GUARDAR PARA MÁS ADELANTE</button>
                    <button onClick={() => removeCartItem(product)} className="font-medium hover:text-red-600">Eliminar</button>
                    </>
                )}
            </div>
            {/* <!-- save for later --> */}
